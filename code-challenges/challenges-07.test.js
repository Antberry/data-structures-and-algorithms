...
const mapTwoToThe = (arr) => {
  let results = arr.map(num => Math.pow(2, num));
  return results;
};
...
const charCode = (arr) => {
  let results = arr.map(element => element.charCodeAt(0));
  return results;
};
...
const extractAbilities = (arr) => {
  let results = arr.map(element => element.ability.name);
  return results;
};
